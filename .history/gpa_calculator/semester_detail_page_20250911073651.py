...
class SemesterDetailPage(ctk.CTkFrame):
    def __init__(
        ...
        # Course list header
        course_header_frame = ctk.CTkFrame(self, fg_color="#1a2b4c", corner_radius=8)
        course_header_frame.pack(fill="x", padx=15, pady=(10, 5))
        course_header_frame.grid_columnconfigure(0, weight=1)  # Course name - flexible
        course_header_frame.grid_columnconfigure(1, minsize=45)  # Credits - very compact for "3.0", "1.0", "5.0"
        course_header_frame.grid_columnconfigure(2, minsize=50)  # Grade - compact for "A+", "A", "B"
        course_header_frame.grid_columnconfigure(3, minsize=40)  # Remove - trash can icon
        
        # Header labels
        ctk.CTkLabel(course_header_frame, text="Course", font=ctk.CTkFont(size=12, weight="bold"), text_color="#ffffff").grid(row=0, column=0, sticky="w", padx=(12, 4), pady=8)
        ctk.CTkLabel(course_header_frame, text="Credits", font=ctk.CTkFont(size=12, weight="bold"), text_color="#ffffff").grid(row=0, column=1, sticky="w", padx=(0, 4), pady=8)
        ctk.CTkLabel(course_header_frame, text="Grade", font=ctk.CTkFont(size=12, weight="bold"), text_color="#ffffff").grid(row=0, column=2, sticky="w", padx=(0, 4), pady=8)
        ...
        # Configure scrollable frame grid columns to match header
        self.subject_list_frame.grid_columnconfigure(0, weight=1)  # Course name - flexible
        self.subject_list_frame.grid_columnconfigure(1, minsize=45)  # Credits - very compact
        self.subject_list_frame.grid_columnconfigure(2, minsize=50)  # Grade - compact
        self.subject_list_frame.grid_columnconfigure(3, minsize=40)  # Remove - trash can icon
    ...
    def _add_subject_row(self, name, credit, grade):
        # Create row with grid layout for precise control
        row_container = ctk.CTkFrame(self.subject_list_frame, height=45, corner_radius=8)
        row_container.pack(fill="x", pady=2, padx=5)
        row_container.pack_propagate(False)
        
        # Configure grid columns
        row_container.grid_columnconfigure(0, weight=1)   # Course name - flexible, takes most space
        row_container.grid_columnconfigure(1, minsize=45) # Credits - very compact for "3.0", "1.0", "5.0"
        row_container.grid_columnconfigure(2, minsize=50) # Grade - compact for "A+", "A", "B"
        row_container.grid_columnconfigure(3, minsize=40) # Remove - trash can icon
        
        # Course name (stretchy - takes most space)
        ...
        name_entry.grid(row=0, column=0, sticky="ew", padx=(12, 4), pady=8)

        # Credit hours (very compact for "3.0", "1.0", "5.0")
        ...
        credit_entry.grid(row=0, column=1, sticky="w", padx=(0, 4), pady=8)

        # Grade (compact for "A+", "A", "B")
        ...
        grade_option.grid(row=0, column=2, sticky="w", padx=(0, 4), pady=8)
        ...
        del_btn.grid(row=0, column=3, sticky="w", padx=(0, 12), pady=8)
