...
class SemesterDetailPage(ctk.CTkFrame):
    def __init__(
        ...
        # Subjects list header - optimized for 360px width
        header_frame = ctk.CTkFrame(self, fg_color="transparent")
        header_frame.pack(fill="x", padx=15, pady=(10, 5))
        header_frame.grid_columnconfigure(0, weight=2)  # Course name gets more space
        header_frame.grid_columnconfigure(1, minsize=45)  # Credits - smaller
        header_frame.grid_columnconfigure(2, minsize=55)  # Grade - smaller
        header_frame.grid_columnconfigure(3, minsize=30)  # Action - smaller
        
        # Header labels
        ctk.CTkLabel(header_frame, text="Course", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db").grid(row=0, column=0, sticky="w", padx=(8, 4))
        ctk.CTkLabel(header_frame, text="Cred", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db").grid(row=0, column=1, sticky="w", padx=(0, 4))
        ctk.CTkLabel(header_frame, text="Grade", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db").grid(row=0, column=2, sticky="w", padx=(0, 4))
        ctk.CTkLabel(header_frame, text="Del", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db").grid(row=0, column=3, sticky="e", padx=(0, 8))
    ...
    def _add_subject_row(self, name, credit, grade):
        # Clean, minimal row design with grid layout
        row = ctk.CTkFrame(self.subject_list_frame, height=40, corner_radius=6)
        ...
        row.grid_columnconfigure(2, minsize=80)
        row.grid_columnconfigure(3, minsize=35)

        # Subject name entry
        name_entry = ctk.CTkEntry(row, height=25, corner_radius=6)
        name_entry.insert(0, str(name) if name is not None else "")
        name_entry.grid(row=0, column=0, sticky="ew", padx=(10, 8), pady=6)

        # Credit hours entry
        credit_entry = ctk.CTkEntry(row, height=25, corner_radius=6)
        credit_entry.insert(0, str(credit) if credit is not None else "")
        credit_entry.grid(row=0, column=1, sticky="ew", padx=(0, 8), pady=6)
        ...
        grade_option.grid(row=0, column=2, sticky="ew", padx=(0, 8), pady=6)
        ...
        del_btn.grid(row=0, column=3, sticky="e", padx=(0, 10), pady=6)
