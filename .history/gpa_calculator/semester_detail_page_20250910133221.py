...
class SemesterDetailPage(ctk.CTkFrame):
    ...
    def _add_subject_row(self, name, credit, grade):
        # Simple row - no complex nesting
        row_container = ctk.CTkFrame(self.subject_list_frame, height=50, corner_radius=8)
        row_container.pack(fill="x", pady=3, padx=5)
        row_container.pack_propagate(False)
        
        # Course name - fixed width to leave room for delete button
        name_entry = ctk.CTkEntry(row_container, width=150, height=30, corner_radius=6, font=ctk.CTkFont(size=12))
        name_entry.insert(0, str(name) if name is not None else "")
        name_entry.pack(side="left", padx=(10, 5), pady=10)
        
        # Credits - fixed width
        credit_entry = ctk.CTkEntry(row_container, width=60, height=30, corner_radius=6, font=ctk.CTkFont(size=12))
        credit_entry.insert(0, str(credit) if credit is not None else "")
        credit_entry.pack(side="left", padx=(0, 5), pady=10)
        ...
        grade_option.pack(side="left", padx=(0, 5), pady=10)
        
        # Delete button - HUGE and impossible to miss
        ...
        del_btn.pack(side="right", padx=(0, 10), pady=10)
