...
class SemesterDetailPage(ctk.CTkFrame):
    ...
    def validate_credit_input(self, event):
        ...
        if value == '.':
            self.show_smooth_message("Please enter a number before the decimal point")
            return
    ...
    def on_credit_key_press(self, event):
        ...
        if event.char:
            if event.char.isalpha():
                self.show_smooth_message("No letters allowed")
            else:
                self.show_smooth_message("No special characters allowed")
        return "break"
    ...
    def on_row_credit_key_press(self, event, credit_entry):
        ...
        if event.char and (event.char.isdigit() or event.char == '.'):
            ...
            if event.char == '.' and '.' in credit_entry.get():
                self.show_smooth_message("Only one decimal point allowed")
                return "break"  # Prevent multiple decimal points
            
            # Check if trying to enter decimal point at the beginning
            if event.char == '.' and not credit_entry.get():
                self.show_smooth_message("Please enter a number before the decimal point")
                return "break"
            
            return
        
        # Block all other characters and show error message
        if event.char:
            self.show_smooth_message("Only numbers allowed. No special characters allowed.")
        return "break"
    ...
    def show_smooth_message(self, text, duration=5000):
        """Show a smooth, modern message with fade effect."""
        ...
        self.current_message.configure(fg_color="#ff4757")
        
        # Fade in effect
        self.fade_in_message()
        
        # Auto-hide after duration
        self.after(duration, self.fade_out_message)
    
    def fade_in_message(self):
        """Fade in the message smoothly."""
        if hasattr(self, 'current_message') and self.current_message:
            # CustomTkinter doesn't support opacity directly, so we use color transition
            self.current_message.configure(fg_color="#ff4757")
