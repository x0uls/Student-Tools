...
class GPACalculatorPage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # Semesters list - directly use scrollable frame for cards
    ...
    def _calculate_semester_gpa(self, subjects):
        """Calculate GPA for a list of subjects using TARUMT rules."""
        if not subjects:
            return 0.0
    ...
    def _calculate_cgpa_tarumt(self, all_subjects):
        ...
        # Create a more robust grouping system that handles name changes
        # Group by a combination of name and credit hours to identify unique subjects
        subject_groups = {}
        for subject in all_subjects:
            # Create a unique key based on name and credit hours
            # This helps identify the same subject even if name changes slightly
            name = subject["name"].strip().lower()
            credit = subject["credit"]
            
            # Try to find existing group by exact match first
            key = f"{name}_{credit}"
            if key not in subject_groups:
                # If no exact match, try to find by credit hours and similar name
                found_existing = False
                for existing_key, existing_subjects in subject_groups.items():
                    existing_credit = existing_subjects[0]["credit"]
                    if existing_credit == credit:
                        # Check if this might be the same subject (same credit, similar name)
                        # This handles cases where subject names are slightly modified
                        existing_name = existing_subjects[0]["name"].strip().lower()
                        if self._is_same_subject(name, existing_name):
                            subject_groups[existing_key].append(subject)
                            found_existing = True
                            break
                
                if not found_existing:
                    subject_groups[key] = [subject]
            else:
        ...
        for subject_key, subjects in subject_groups.items():
            # Check if this subject was ever failed
            ...
            # Add credits, but exclude failed subject credits only once
            if has_failed:
                # Add credits for all attempts except one (to exclude failed credits once)
                for i, subject in enumerate(subjects):
                    if i == 0:  # Skip first attempt (exclude failed credits once)
                        continue
                    total_credits += subject["credit"]
            else:
                # No failures, add all credits normally
                for subject in subjects:
                    total_credits += subject["credit"]
        
        return total_points / total_credits if total_credits > 0 else 0.0

    def _is_same_subject(self, name1, name2):
        """Check if two subject names likely refer to the same subject."""
        # Remove common variations and check similarity
        name1_clean = name1.replace("&", "and").replace("-", " ").replace("_", " ")
        name2_clean = name2.replace("&", "and").replace("-", " ").replace("_", " ")
        
        # Split into words and check for significant overlap
        words1 = set(name1_clean.split())
        words2 = set(name2_clean.split())
        
        # If more than 50% of words overlap, consider it the same subject
    ...
    def _update_total_cgpa(self, save_data=True):
        """Calculate CGPA from all semesters using TARUMT rules."""
    ...
    def _update_chart(self):
        """Update chart data if chart page exists."""
