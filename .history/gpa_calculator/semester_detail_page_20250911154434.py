...
class SemesterDetailPage(ctk.CTkFrame):
    ...
    def on_credit_key_press(self, event):
        """Handle credit field input with auto-decimal formatting."""
        # Allow backspace, delete, tab, enter, and arrow keys
        if event.keysym in ['BackSpace', 'Delete', 'Tab', 'Return', 'Left', 'Right', 'Up', 'Down']:
            return
        
        # Handle digit input with auto-decimal formatting
        if event.char and event.char.isdigit():
            # Check if trying to enter 0
            if event.char == '0':
                self.show_smooth_message("Only numbers greater than 0 are allowed", duration=5000)
                return "break"
            
            current_value = self.credit_entry.get()
            
            # If field is empty, allow the digit
            if not current_value:
                # Block the character and format manually
                self.after(1, lambda: self.format_credit_with_decimal(event.char))
                return "break"
            
            # If field already has content, replace it with new digit
            if current_value.isdigit() or (current_value.endswith('.0') and current_value[:-2].isdigit()):
                # Block the character and format manually
                self.after(1, lambda: self.format_credit_with_decimal(event.char))
                return "break"
        ...
        if event.char:
            # Cancel any existing message timer to prevent clearing
            if hasattr(self, 'message_timer'):
                self.after_cancel(self.message_timer)
            
            if event.char.isalpha():
                self.show_smooth_message("Only numbers allowed!", duration=5000)
            else:
                self.show_smooth_message("No special characters allowed!", duration=5000)
        return "break"

    def format_credit_with_decimal(self, digit):
        """Format the credit field with automatic .0 decimal."""
        # Clear the field and set the new digit with .0
        self.credit_entry.delete(0, "end")
        self.credit_entry.insert(0, f"{digit}.0")
        
        # Hide any error messages since we have valid input
        self.hide_message()


    def on_row_credit_key_press(self, event, credit_entry):
        """Handle row credit field input with auto-decimal formatting."""
        # Allow backspace, delete, tab, enter, and arrow keys
        if event.keysym in ['BackSpace', 'Delete', 'Tab', 'Return', 'Left', 'Right', 'Up', 'Down']:
            return
        ...
        if event.char and event.char.isdigit():
            ...
            current_value = credit_entry.get()
            
            # If field is empty, allow the digit
            if not current_value:
                # Schedule the auto-decimal formatting after the key is processed
                self.after(1, lambda: self.format_row_credit_with_decimal(event.char, credit_entry))
                return
            
            # If field already has content, replace it with new digit
            if current_value.isdigit() or (current_value.endswith('.0') and current_value[:-2].isdigit()):
                # Replace existing content with new digit
                self.after(1, lambda: self.format_row_credit_with_decimal(event.char, credit_entry))
                return "break"
    ...
    def format_row_credit_with_decimal(self, digit, credit_entry):
        """Format the row credit field with automatic .0 decimal."""
        # Clear the field and set the new digit with .0
        credit_entry.delete(0, "end")
        credit_entry.insert(0, f"{digit}.0")
