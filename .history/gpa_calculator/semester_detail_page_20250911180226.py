...
class SemesterDetailPage(ctk.CTkFrame):
    """Page for managing individual semester subjects and calculating semester GPA."""
    ...
    def __init__(
        ...
        self.subjects = []  # List of subject widget references
        self.configure(fg_color="transparent")
        
        # Create header with back button, title, and GPA display
        ...
        # Create scrollable frame for subject list
    ...
    def add_subject(self):
        """Add a new subject from the form with validation."""
    ...
    def handle_credit_input(self, event, credit_entry=None):
        """Handle credit input with auto-decimal formatting and validation."""
    ...
    def _format_credit_entry(self, entry, digit):
        """Format credit entry with .0 decimal and update GPA if needed."""
        entry.delete(0, "end")
    ...
    def _add_subject_row(self, name, credit, grade):
        """Create a new subject row with editable fields and remove button."""
    ...
    def remove_subject(self, row_container):
        """Remove a subject row and update GPA."""
    ...
    def _update_gpa(self, update_main_page=True):
        """Recalculate semester GPA and update main page if requested."""
        subjects_data = self.get_subjects_data()
        gpa = self._calculate_gpa(subjects_data)
        self.gpa_label.configure(text=f"GPA: {gpa:.4f}")
    ...
    def _calculate_gpa(self, subjects_data):
        """Calculate semester GPA from subjects data."""
    ...
    def get_subjects_data(self):
        """Extract subject data from UI widgets for storage."""
        subjects_data = []
        for subj in self.subjects:
            try:
    ...
    def show_error(self, text, duration=3000):
        """Display error message with auto-hide timer."""
    ...
    def hide_message(self):
        """Hide current error message and clean up timer."""
        if hasattr(self, 'message_timer'):
            self.after_cancel(self.message_timer)
