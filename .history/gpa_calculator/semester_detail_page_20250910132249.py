...
class SemesterDetailPage(ctk.CTkFrame):
    def __init__(
        ...
        # Subjects list header - simple pack layout
        header_frame = ctk.CTkFrame(self, fg_color="transparent")
        header_frame.pack(fill="x", padx=15, pady=(10, 5))
        
        # Header labels using pack with fixed widths
        course_label = ctk.CTkLabel(header_frame, text="Course Name", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db")
        course_label.pack(side="left", padx=(10, 0))
        
        credits_label = ctk.CTkLabel(header_frame, text="Credits", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db")
        credits_label.pack(side="left", padx=(20, 0))
        
        grade_label = ctk.CTkLabel(header_frame, text="Grade", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db")
        grade_label.pack(side="left", padx=(15, 0))
        
        action_label = ctk.CTkLabel(header_frame, text="Action", font=ctk.CTkFont(size=11, weight="bold"), text_color="#d1d5db")
        action_label.pack(side="right", padx=(0, 10))
    ...
    def _add_subject_row(self, name, credit, grade):
        # Row design with exact grid matching for perfect alignment
        ...
        row.grid_columnconfigure(1, minsize=50)
        row.grid_columnconfigure(2, minsize=60)
        row.grid_columnconfigure(3, minsize=35)

        # Subject name entry - matches header padding exactly
        name_entry = ctk.CTkEntry(row, height=22, corner_radius=4, font=ctk.CTkFont(size=10))
        name_entry.insert(0, str(name) if name is not None else "")
        name_entry.grid(row=0, column=0, sticky="ew", padx=(10, 5), pady=5)

        # Credit hours entry - matches header padding exactly
        credit_entry = ctk.CTkEntry(row, height=22, corner_radius=4, font=ctk.CTkFont(size=10))
        credit_entry.insert(0, str(credit) if credit is not None else "")
        credit_entry.grid(row=0, column=1, sticky="ew", padx=(0, 5), pady=5)

        # Grade dropdown - matches header padding exactly
        ...
        grade_option.grid(row=0, column=2, sticky="ew", padx=(0, 5), pady=5)

        # Delete button - matches header padding exactly
        ...
        del_btn.grid(row=0, column=3, sticky="e", padx=(0, 10), pady=5)
