...
class SemesterDetailPage(ctk.CTkFrame):
    ...
    def _add_subject_row(self, name, credit, grade):
        ...
        name_entry.pack(side="left", padx=(10, 0), pady=5, fill="x", expand=True)

        # Credit hours entry - fixed width to match header
        credit_entry = ctk.CTkEntry(row, width=60, height=22, corner_radius=4, font=ctk.CTkFont(size=11))
        credit_entry.insert(0, str(credit) if credit is not None else "")
        credit_entry.pack(side="left", padx=(20, 0), pady=5)

        # Grade dropdown - fixed width to match header
        ...
        grade_option.pack(side="left", padx=(15, 0), pady=5)
        ...
        del_btn.pack(side="right", padx=(0, 10), pady=5)
